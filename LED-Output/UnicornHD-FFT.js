...
const { size, color } = require('./../config/config.js');
...
function UnicornHD_FFT(bands) { //setts how many bands on the display. must be evenly divisible by size.x
    this.bands = bands;
...
UnicornHD_FFT.prototype.displayOut = function (fftArr = []) {
    ...
    resetMatrix(color.aqua, color.pastel_yellow, color.purple); //resets all the bands before turning off the empty value LEDS
